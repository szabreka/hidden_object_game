...
top_5_entries = sorted_filtered_object_data[:5]
top_5_coords = coord_data_filtered[:5]

image = cv2.imread(source)
image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

top_5_coordinates= []
def get_xy_data(object_data):
    for n in range(0,5):
        label_data = object_data[n][0]
        x_data = object_data[n][2]
        y_data = object_data[n][3]
        top_5_coordinates.append((label_data, x_data,y_data))
